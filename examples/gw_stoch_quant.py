...
class StochasticGridworld(Gridworld):
    ...
    In comparison with `gw_stoch_qual/StochasticGridworld`, the `delta` method is different. In this case,
    we assign probabilities to each state.
    There are two options to mark the gridworld as stochastic:
        * Define `__init__` function and set `self.deterministic = False` and `qualitative = False`
            after a call to `super(...).__init__(...)`.
        * Pass `deterministic = False, qualitative=False` during object creation.
    """
