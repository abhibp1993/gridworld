...
class StochasticGridworld(TSGenerator):
    """
    `StochasticGridworld` represents a quantitative stochastic gridworld with no obstacles and one player.

    To make gridworld stochastic, set `DETERMINISTIC` to `False`.
    To make gridworld quantitative, set `QUALITATIVE` to `False`.
    ...
    def delta(self, state, act):
        ...
        :return: An iterable (list/set) over next states. Each returned state must be in self.states().
        """
        # Decouple state
        row, col = state
